...
cursor = db.cursor()
# 处理vulnerability表中的漏洞和os的关系，按照目前的数据源，仅有CVE的
# 只关心type为1或者2的漏洞
...
def UpdateRelation(CVEId, APPId):
    print "inserting..."
    insertVertex(APPId, 3) # 我知道os还没插
    # vulnerability大概率不用插
    outId = getVertexId(CVEId, 1)
    inId = getVertexId(APPId, 3)
...
try:
    cursor.execute("""
    select number, product from vulnerablesoftwarelist where product like '%/a:%';
    """)
    data = cursor.fetchall()
    for item in data:
        CVEId = getCVEId(item[0])
        APPId = getAPPId(item[1])
        if CVEId == None or APPId == None:
            continue
        else:
            UpdateRelation(CVEId, APPId)
except:
    err_log()
