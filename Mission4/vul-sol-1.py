...
def getSolId(i):
    sql = """
    select id from solution where type = 1 and cveId = "{cveId}";
    """.format(cveId = i)
    try:
        cursor.execute(sql)
        res = cursor.fetchone()
...
def UpdateRelation(CVEId, SolId):
    print "inserting..."
...
try:
    cursor.execute("""
    select id, cveId from vulnerability where type = 1 or type = 2;
    """)
    data = cursor.fetchall()
    for item in data:
        CVEId = item[0]
        SolId = getSolId(item[1])
        if CVEId == None or SolId == None:
            continue
        else:
            UpdateRelation(CVEId, SolId)
except:
    err_log()
