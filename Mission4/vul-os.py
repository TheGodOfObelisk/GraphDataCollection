...
def getOSId(i):
    sql = """
    select id from OS where Name = "{name}";
    """.format(name = i)
    try:
        cursor.execute(sql)
        res = cursor.fetchone()
...
def UpdateRelation(CVEId, OSId):
    print "inserting..."
    insertVertex(OSId, 7) # 我知道os还没插
    # vulnerability大概率不用插
...
try:
    cursor.execute("""
    select number, product from vulnerablesoftwarelist where product like '%/o:%';
    """)
    data = cursor.fetchall()
    for item in data:
        CVEId = getCVEId(item[0])
        OSId = getOSId(item[1])
        if CVEId == None or OSId == None:
            continue
        else:
            UpdateRelation(CVEId, OSId)
except:
    err_log()
